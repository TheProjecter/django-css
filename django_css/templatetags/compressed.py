...
from django_css.utils import media_root, media_url, needs_update, filter_css, filter_js, get_output_filename, get_version, get_version_from_file, compile_css
    ...
    def render(self, context):
        ...
        if settings.COMPRESS:
            ...
            if settings.COMPRESS_AUTO:
                ...
                css = compile_css(css,u)
                # Get the correct version number
                version = get_version(css['source_filenames'])
                if u:
                    filter_css(css,0)
            else:
